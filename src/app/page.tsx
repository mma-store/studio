...
export default function SaaSLandingPage() {
  return (
    <div className="min-h-screen bg-white text-slate-900 font-almarai selection:bg-blue-100 overflow-x-hidden" dir="rtl">
      ...
      <section id="sectors" className="py-32 bg-[#0A192F] text-white relative overflow-hidden">
        <div className="container mx-auto px-6 flex flex-col lg:flex-row items-center gap-24 relative z-10">
          <div className="flex-1 space-y-8">
             <div className="space-y-4">
                <h2 className="text-4xl md:text-5xl font-black leading-tight">منصتنا تدعم <span className="text-blue-400">مختلف القطاعات</span></h2>
                <p className="text-lg text-slate-400 font-medium">سواء كنت تدير محلاً صغيراً أو مجمعاً تجارياً كبيراً، نظامنا يتكيف مع احتياجاتك.</p>
             </div>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  "قطع غيار الدراجات والسيارات", 
                  "محلات الأزياء والملابس", 
                  "معارض الإلكترونيات والموبايل", 
                  "المتاجر العامة والأسواق", 
                  "الصيدليات ومحلات التجميل", 
                  "تجهيزات المكاتب والأثاث"
                ].map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-5 rounded-2xl bg-white/5 border border-white/10 font-bold text-sm hover:bg-white/10 transition-colors">
                    <Check className="h-5 w-5 text-blue-400 shrink-0" />
                    <span>{item}</span>
                  </div>
                ))}
             </div>
             <Button variant="outline" className="h-14 rounded-2xl px-10 border-white/20 text-white hover:bg-white/10 font-black">اكتشف الحلول المخصصة</Button>
          </div>
          <div className="flex-1 relative h-[550px] w-full rounded-[48px] overflow-hidden shadow-2xl border-8 border-white/5">
             <Image 
              src="https://picsum.photos/seed/industries/800/600" 
              alt="Sector Solutions" 
              fill 
        ...
        <div className="absolute -left-20 -top-20 h-96 w-96 bg-blue-600/10 rounded-full blur-[100px]" />
      </section>
      ...
      <section id="pricing" className="py-32 bg-slate-50">
        <div className="container mx-auto px-6 space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             {[
               { name: "الباقة الأساسية", price: "0", desc: "مثالية للمشاريع الجديدة والناشئة", features: ["1 متجر إلكتروني", "حتى 10 منتجات", "نظام POS قياسي", "تقارير مبيعات مبسطة"], cta: "ابدأ مجاناً", active: false },
               { name: "باقة الأعمال", price: "15,000", desc: "للمحلات والمجمعات الطموحة للنمو", features: ["منتجات غير محدودة", "تقارير مالية متقدمة", "3 حسابات للموظفين", "تنبيهات المخزون", "إدارة ديون الزبائن"], cta: "اشترك الآن", active: true },
               { name: "باقة المؤسسات", price: "35,000", desc: "للمؤسسات التي تحتاج دعماً مخصصاً", features: ["دومين خاص (Custom Domain)", "دعم فني مخصص", "تحديثات ذات أولوية", "إدارة فروع متعددة", "تكاملات برمجية خاصة"], cta: "تواصل معنا", active: false }
             ].map((plan, i) => (
      ...
      <section id="faq" className="py-32 bg-white">
        <div className="container mx-auto px-6 max-w-3xl space-y-16">
          <div className="text-center space-y-4">
             <h2 className="text-4xl font-black text-[#0A192F]">الأسئلة الشائعة</h2>
             <p className="text-slate-500 font-medium">كل ما تود معرفته عن منصتنا السحابية.</p>
          </div>
          <Accordion type="single" collapsible className="w-full space-y-4">
            {[
              { q: "كم يستغرق تجهيز المتجر وبدء البيع؟", a: "يمكنك إنشاء متجرك وإضافة أول منتج في أقل من 60 ثانية باستخدام معالج التسجيل الذكي الخاص بنا." },
              { q: "هل يمكنني استخدام طابعتي الحالية مع نظام الـ POS؟", a: "نعم، نظامنا يدعم كافة أنواع الطابعات الحرارية القياسية (58mm و 80mm) بالإضافة إلى طابعات A4 عبر أي جهاز متصل." },
              { q: "هل بيانات متجري وزبائني آمنة؟", a: "نحن نستخدم تشفيراً سحابياً بمواصفات عالمية، مع عزل تام لبيانات كل تاجر لضمان خصوصية مطلقة وأمان فائق." },
              { q: "هل يمكنني الترقية أو إلغاء الاشتراك في أي وقت؟", a: "بالتأكيد! يمكنك تغيير خطة اشتراكك أو إيقافها في أي لحظة مباشرة من لوحة التحكم الخاصة بك دون أي شروط معقدة." }
            ].map((faq, i) => (
              <AccordionItem key={i} value={`item-${i}`} className="border-none px-8 rounded-[24px] bg-slate-50 py-2">
                <AccordionTrigger className="font-black text-right hover:no-underline text-lg text-[#0A192F]">{faq.q}</AccordionTrigger>
                <AccordionContent className="font-bold text-slate-500 leading-relaxed text-base pt-2">{faq.a}</AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-20 text-right">
             <div className="col-span-1 md:col-span-1 space-y-8">
                ...
                <div className="flex gap-4">
                   {/* Social Placeholders */}
                   {[1, 2, 3, 4].map(i => <div key={i} className="h-10 w-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all cursor-pointer"><Smartphone className="h-4 w-4" /></div>)}
                </div>
             </div>
             <div className="space-y-6">
                ...
                <ul className="space-y-4 text-sm font-bold">
                   ...
                   <li><a href="#" className="hover:text-blue-400 transition-colors">الأسعار</a></li>
             ...
             <div className="space-y-6">
                ...
                <ul className="space-y-4 text-sm font-bold">
                   <li><a href="#" className="hover:text-blue-400 transition-colors">مركز المساعدة</a></li>
             ...
             <div className="space-y-6">
                ...
                <ul className="space-y-4 text-sm font-bold">
                   ...
                   <li><a href="#" className="hover:text-blue-400 transition-colors">اتفاقية مستوى الخدمة</a></li>
                </ul>
             </div>
          </div>
          <div className="pt-10 border-t border-white/5 flex flex-col items-center gap-6">
             <div className="flex flex-col items-center gap-2">
                <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-40">All rights reserved © {new Date().getFullYear()}</p>
